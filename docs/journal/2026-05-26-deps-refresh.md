...
| wasmtime                       | 26      | 45        | Per AGENTS.md and the brief: wasmtime is pinned at the 26.x train and bumping requires a journal entry. The 19-minor jump touches the engine API our `dyniak::mapreduce::wasm` module rests on. Deferred to a dedicated pass; the existing 26.x-only RUSTSECs are tracked separately.                                                                |
