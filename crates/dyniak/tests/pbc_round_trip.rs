...
//!
//! Spins up [`dyniak::serve_pbc`] over a real `tokio::net::TcpListener`
...
//! rather than [`dyniak::datastore::NoxuDatastore`] so they run
//! without the noxu checkout next door.
...
async fn exchange_server_info(r: &mut Reader, w: &mut Writer) {
    ...
    assert!(
        version_str.starts_with("dyniak "),
