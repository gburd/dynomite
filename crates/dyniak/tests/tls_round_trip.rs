...
//!   TLS via `dyniak::serve_pbc_tls`, the client connects via
...
//!   terminates TLS via `dyniak::serve_http_tls`, a manual
...
use dyniak::proto::pb::{read_frame, write_frame, Frame, MessageCode, RpbPingResp};
...
async fn http_ping_round_trips_over_tls() {
    ...
    // dyniak test surface; the wire shape is a textbook GET.
