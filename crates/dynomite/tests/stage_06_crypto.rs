...
fn aes_round_trip_known_lengths() {
    ...
    for n in [0usize, 1, 15, 16, 17, 31, 32, 33, 4096] {
        ...
        assert_eq!(cipher.len() % AES_BLOCK_SIZE, AES_IV_LEN % AES_BLOCK_SIZE);
...
proptest! {
    ...
    fn aes_round_trip_property(msg in prop::collection::vec(any::<u8>(), 0..4096)) {
        let key = Crypto::generate_aes_key().unwrap();
        let cipher = Crypto::aes_encrypt(&msg, &key).unwrap();
        prop_assert!(cipher.len() >= AES_IV_LEN + AES_BLOCK_SIZE);
        let plain = Crypto::aes_decrypt(&cipher, &key).unwrap();
...
fn dyn_aes_encrypt_msg_round_trip() {
    ...
    assert!(n >= AES_IV_LEN + AES_BLOCK_SIZE);
...
/// Frozen-output cross-version compatibility test.
///
/// The fixture under `tests/fixtures/crypto/` is generated by
/// `examples/gen_crypto_fixture.rs`. The test asserts that the
/// committed ciphertext still decrypts to the committed plaintext
/// using the committed AES key, then re-encrypts with a fresh IV and
/// decrypts again to confirm round-trip stability.
#[test]
fn cross_version_cipher_round_trip() {
    ...
    assert_ne!(
        fresh_cipher, cipher,
        "fresh IV must produce a different ciphertext"
    );
    let recovered_again = decrypt_to_vec(&fresh_cipher, &key).unwrap();
